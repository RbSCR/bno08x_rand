...
#include "bno08x_try_sensor_status/bno08x_try_sensor_status.hpp"
...
TrySensorStatusNode::TrySensorStatusNode() : Node("bno08x_try_sensor_status") {
  ...
  if (this->imu_rate_ < this->magnetic_field_rate_) {
    this->poll_timer_ = this->create_wall_timer(
      ...
      std::bind(&TrySensorStatusNode::poll_timer_callback, this));
  } else {
    this->poll_timer_ = this->create_wall_timer(
  ...
  watchdog_->set_callback([this]() {
    RCLCPP_ERROR(
      this->get_logger(), "Watchdog timeout! No data received from sensor. Resetting...");
    this->reset();
  });
  watchdog_->start();

  RCLCPP_INFO(this->get_logger(), "BNO08x_try_sensor_status node started.");
}

TrySensorStatusNode::~TrySensorStatusNode() {
  delete watchdog_;
  delete bno08x_;
 ...
 */
void TrySensorStatusNode::init_comms() {
  RCLCPP_INFO(this->get_logger(), "Init comms");
  ...
  if (i2c_enabled) {
    ...
    try {
      ...
      RCLCPP_ERROR(this->get_logger(), "Failed to create I2CInterface: %s", e.what());
    ...
    try {
      ...
      RCLCPP_ERROR(this->get_logger(), "UART Interface not implemented: %s", e.what());
    ...
    try {
      ...
      RCLCPP_ERROR(this->get_logger(), "SPI Interface not implemented: %s", e.what());
  ...
  RCLCPP_INFO(this->get_logger(), "Comms initialized");
}
 ...
 *
 */
void TrySensorStatusNode::init_parameters() {
 ...
 *
 */
void TrySensorStatusNode::init_sensor() {
  ...
  try {
    bno08x_ = new BNO08x(
      comm_interface_,
      std::bind(
        &TrySensorStatusNode::sensor_callback, this, std::placeholders::_1, std::placeholders::_2),
      this);
  } catch (const std::bad_alloc & e) {
    RCLCPP_ERROR(this->get_logger(), "Failed to allocate memory for BNO08x object: %s", e.what());
 ...
 *
 */
void TrySensorStatusNode::sensor_callback(void * cookie, sh2_SensorValue_t * sensor_value) {
 ...
 */
void TrySensorStatusNode::poll_timer_callback() {
  {
...
int main(int argc, char ** argv) {
  ...
  std::shared_ptr<TrySensorStatusNode> try_node = std::make_shared<TrySensorStatusNode>();
  rclcpp::spin(try_node);
  rclcpp::shutdown();
